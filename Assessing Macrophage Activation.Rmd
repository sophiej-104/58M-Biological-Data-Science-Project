...
## Comparing Log vs Logicle Transformation

### Log Channel FS vs SS Plots

```{r plot log channel data, out.width="100%"}

log_1A <- ggcyto(fcm1A_comp_trans, aes(x = "FS.Log", y = "SS.Log")) + geom_hex(bins = 100) + theme_bw()
log_1A <- as.ggplot(log_1A)

log_1B <- ggcyto(fcm1B_comp_trans, aes(x = "FS.Log", y = "SS.Log")) + geom_hex(bins = 100) + theme_bw()
log_1B <- as.ggplot(log_1B)

log_2A <- ggcyto(fcm2A_comp_trans, aes(x = "FS.Log", y = "SS.Log")) + geom_hex(bins = 100) + theme_bw()
log_2A <- as.ggplot(log_2A)

log_2B <- ggcyto(fcm2B_comp_trans, aes(x = "FS.Log", y = "SS.Log")) + geom_hex(bins = 100) + theme_bw()
log_2B <- as.ggplot(log_2B)

ggpubr::ggarrange(log_1A, log_1B, log_2A, log_2B)

```

### Logicle FS vs SS Plots

```{r plot logicle transformed data, out.width="100%"}

logicle_1A <- ggcyto(fcm1A_comp_trans, aes(x = "FS.Lin", y = "SS.Lin")) + geom_hex(bins = 100) + theme_bw()
logicle_1A <- as.ggplot(logicle_1A) #converting to a ggplot so all four plots can be shown together using ggarrange

logicle_1B <- ggcyto(fcm1B_comp_trans, aes(x = "FS.Lin", y = "SS.Lin")) + geom_hex(bins = 100) + theme_bw()
logicle_1B <- as.ggplot(logicle_1B)

logicle_2A <- ggcyto(fcm2A_comp_trans, aes(x = "FS.Lin", y = "SS.Lin")) + geom_hex(bins = 100) + theme_bw()
logicle_2A <- as.ggplot(logicle_2A)

logicle_2B <- ggcyto(fcm2B_comp_trans, aes(x = "FS.Lin", y = "SS.Lin")) + geom_hex(bins = 100) + theme_bw()
logicle_2B <- as.ggplot(logicle_2B)

ggpubr::ggarrange(logicle_1A, logicle_1B, logicle_2A, logicle_2B) #plotting the four plots together
...
fcm1A_summary_logicle <- as.data.frame(fcm1A_summary_logicle[3, 10]) %>% mutate(., Mean = mean_fcm1A_logicle) %>%
  mutate(., Cell_Count = cell_count_1A_logicle) %>% rename(Median = `fcm1A_summary_logicle[3, 10]`) %>% mutate(., LPS_Stimulation = "-") %>% mutate(., Anti_TNF_Stain = "-")
...
fcm1B_summary_logicle <- as.data.frame(fcm1B_summary_logicle[3, 10]) %>% mutate(., Mean = mean_fcm1B_logicle) %>%
  mutate(., Cell_Count = cell_count_1B_logicle) %>% rename(Median = `fcm1B_summary_logicle[3, 10]`) %>% mutate(., LPS_Stimulation = "-") %>% mutate(., Anti_TNF_Stain = "+")
...
fcm2A_summary_logicle <- as.data.frame(fcm2A_summary_logicle[3, 10]) %>% mutate(., Mean = mean_fcm2A_logicle) %>%
  mutate(., Cell_Count = cell_count_2A_logicle) %>% rename(Median = `fcm2A_summary_logicle[3, 10]`) %>% mutate(., LPS_Stimulation = "+") %>% mutate(., Anti_TNF_Stain = "-")
...
fcm2B_summary_logicle <- as.data.frame(fcm2B_summary_logicle[3, 10]) %>% mutate(., Mean = mean_fcm2B_logicle) %>%
  mutate(., Cell_Count = cell_count_2B_logicle) %>% rename(Median = `fcm2B_summary_logicle[3, 10]`) %>% mutate(., LPS_Stimulation = "+") %>% mutate(., Anti_TNF_Stain = "+")

logicle_summary <- rbind(fcm1A_summary_logicle, fcm1B_summary_logicle, fcm2A_summary_logicle, fcm2B_summary_logicle)
logicle_summary <- logicle_summary[, c(4, 5, 3, 2, 1)]
