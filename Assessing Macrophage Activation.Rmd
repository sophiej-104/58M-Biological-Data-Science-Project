...
<!-- Sample Summaries: -->
<!-- Sample 1: unstimulated cells kept in culture medium -->
<!-- Sample 2: cells treated with 100 ng/ml LPS from E.coli -->
<!-- Sample 3: mCherry fluorescent E.coli bacteria -->
<!-- Samples marked A: use secondary antibody alone, without the addition of the primary antibody, as a control. -->
<!-- Samples marked B: use both primary and secondary antibodies -->

```{r reading in the data}
file1A <- "./data-raw/example-data/1A. Unstimulated Secondary alone.fcs"
fcm1A <- read.FCS(file1A, alter.names = TRUE)

file1B <- "./data-raw/example-data/1B. Unstimulated Primary and Secondary.fcs"
fcm1B <- read.FCS(file1B, alter.names = TRUE)

file2A <- "./data-raw/example-data/2A. LPS Secondary alone.fcs"
fcm2A <- read.FCS(file2A, alter.names = TRUE)

file2B <- "./data-raw/example-data/2B. LPS Primary and Secondary.fcs"
fcm2B <- read.FCS(file2B, alter.names = TRUE)

file3A <- "./data-raw/example-data/3A. mCherry+ Bacteria Secondary alone.fcs"
fcm3A <- read.FCS(file3A, alter.names = TRUE)

file3B <- "./data-raw/example-data/3B. mCherry+ Bacteria Primary and Secondary.fcs"
fcm3B <- read.FCS(file3B, alter.names = TRUE)
...
fcm3Adf <- data.frame(fcm3A@exprs)
fcm3Bdf <- data.frame(fcm3B@exprs)
...
```{r scatter plot of 3a, include=FALSE}
...
fcm3A_gate_r1 <- Subset(fcm3A, gate_r1)

fcm3A_gate_r1df <- data.frame(fcm3A_gate_r1@exprs)

fig_gate_3A <- ggplot(data = fcm3Adf, aes(x = FS.Lin, y = SS.Lin)) +
  geom_point(alpha = 0.5) +
  geom_point(data = fcm3A_gate_r1df, 
             aes(x = FS.Lin, y = SS.Lin), color = "red") 

```

```{r scatter plot of 3b, include=FALSE}
...
fcm3B_gate_r1 <- Subset(fcm3B, gate_r1)

fcm3B_gate_r1df <- data.frame(fcm3B_gate_r1@exprs)

fig_gate_3B <- ggplot(data = fcm3Bdf, aes(x = FS.Lin, y = SS.Lin)) +
  geom_point(alpha = 0.5) +
  geom_point(data = fcm3B_gate_r1df, 
             aes(x = FS.Lin, y = SS.Lin), color = "red") 
...
ggpubr::ggarrange(fig_gate_1A, fig_gate_2A, fig_gate_3A, labels = c("A", "B", "C"), cols = 3)
