...
# combining the two datasets that will be plotted together 
# first make new column so they can be distinguished from each other for plot

fcm1A_gate_r1df$staining <- 'nostain'
fcm1B_gate_r1df$staining <- 'stained'

fcm1_gate_dfplot <- rbind(fcm1A_gate_r1df, fcm1B_gate_r1df)

hist_log_1 <- ggplot(fcm1_gate_dfplot, aes(x = FL.1.Log, fill = staining)) + geom_histogram(aes(y = ..count..), alpha= 0.5, position = 'identity') + scale_x_log10() +
  ylab("Count") +
  xlab("FITC Log") +
  scale_fill_discrete(name = "", labels = c(expression("no anti-TNF"*alpha*~"staining"), expression("anti-TNF"*alpha*~"staining")))
hist_log_1

fcm2A_gate_r1df$staining <- 'nostain'
fcm2B_gate_r1df$staining <- 'stained'

fcm2_gate_dfplot <- rbind(fcm2A_gate_r1df, fcm2B_gate_r1df)

hist_log_2 <- ggplot(fcm2_gate_dfplot, aes(x = FL.1.Log, fill = staining)) + geom_histogram(aes(y = ..count..), alpha= 0.5, position = 'identity') + scale_x_log10() +
  ylab("Count") +
  xlab("FITC Log") +
  scale_fill_discrete(name = "", labels = c(expression("no anti-TNF"*alpha*~"staining"), expression("anti-TNF"*alpha*~"staining")))
hist_log_2

TNFalpha_detection <- ggpubr::ggarrange(log_1A_gate, hist_log_1, hist_log_2, labels = "AUTO")
TNFalpha_detection
