...
This step is carried out in order to visualise the data more easily as many of the channels are not in a log format, the code here transforms the data into a "logicle" format. 
...
```{r visualise transformed data, include=FALSE}
...
```{r setting the initial gating regions for log data, out.width="100%"}
...
log_1A_gate <- ggcyto(fcm1A_comp_trans, aes(x = "FS.Log", y = "SS.Log")) + geom_hex(bins = 100) + theme_bw() + geom_gate(gate_r1) + geom_stats()
...
log_1B_gate <- ggcyto(fcm1B_comp_trans, aes(x = "FS.Log", y = "SS.Log")) + geom_hex(bins = 100) + theme_bw() + geom_gate(gate_r1) + geom_stats()
...
log_2A_gate <- ggcyto(fcm2A_comp_trans, aes(x = "FS.Log", y = "SS.Log")) + geom_hex(bins = 100) + theme_bw() + geom_gate(gate_r1) + geom_stats()
...
log_2B_gate <- ggcyto(fcm2B_comp_trans, aes(x = "FS.Log", y = "SS.Log")) + geom_hex(bins = 100) + theme_bw() + geom_gate(gate_r1) + geom_stats()
...
ggpubr::ggarrange(log_1A_gate, log_1B_gate, log_2A_gate, log_2B_gate)

```

```{r setting the initial gating regions for logicle data, out.width="100%"}

poly_logicle <- matrix(c(3.5, 2.5,                  
                  3.5, 4.5,
                  4.5, 4.5,
                  4.5, 2.5), 
                ncol = 2, 
...
colnames(poly_logicle) <- c("FS.Lin","SS.Lin")

# creating polygongate

gate_r2 <- polygonGate(poly_logicle, filterId = "gate_r2")

# plotting samples with gating

logicle_1A_gate <- ggcyto(fcm1A_comp_trans, aes(x = "FS.Lin", y = "SS.Lin")) + geom_hex(bins = 100) + theme_bw() + geom_gate(gate_r2) + geom_stats()
logicle_1A_gate <- as.ggplot(logicle_1A_gate)

fcm1A_gate_r2 <- Subset(fcm1A_comp_trans, gate_r2)
fcm1A_gate_r2df <- data.frame(fcm1A_gate_r2@exprs)

logicle_1B_gate <- ggcyto(fcm1B_comp_trans, aes(x = "FS.Lin", y = "SS.Lin")) + geom_hex(bins = 100) + theme_bw() + geom_gate(gate_r2) + geom_stats()
logicle_1B_gate <- as.ggplot(logicle_1B_gate)

fcm1B_gate_r2 <- Subset(fcm1B_comp_trans, gate_r2)
fcm1B_gate_r2df <- data.frame(fcm1B_gate_r2@exprs)

logicle_2A_gate <- ggcyto(fcm2A_comp_trans, aes(x = "FS.Lin", y = "SS.Lin")) + geom_hex(bins = 100) + theme_bw() + geom_gate(gate_r2) + geom_stats()
logicle_2A_gate <- as.ggplot(logicle_2A_gate)

fcm2A_gate_r2 <- Subset(fcm2A_comp_trans, gate_r2)
fcm2A_gate_r2df <- data.frame(fcm2A_gate_r2@exprs)

logicle_2B_gate <- ggcyto(fcm2B_comp_trans, aes(x = "FS.Lin", y = "SS.Lin")) + geom_hex(bins = 100) + theme_bw() + geom_gate(gate_r2) + geom_stats()
logicle_2B_gate <- as.ggplot(logicle_2B_gate)

fcm2B_gate_r2 <- Subset(fcm2B_comp_trans, gate_r2)
fcm2B_gate_r2df <- data.frame(fcm2B_gate_r2@exprs)

ggpubr::ggarrange(logicle_1A_gate, logicle_1B_gate, logicle_2A_gate, logicle_2B_gate)

```


```{r log histograms, out.width="100%"}
...
hist_log_1A <- ggcyto(fcm1A_gate_r1, aes(x = FL.1.Log)) + geom_histogram() + scale_x_log10() +
  ylab("Count") +
  xlab("FITC Log") +
  theme_bw() + geom_gate(gate_r3) + geom_stats(adjust = 0.5)
...
hist_log_1B <- ggcyto(fcm1B_gate_r1, aes(x = FL.1.Log)) + geom_histogram() + scale_x_log10() +
  ylab("Count") +
  xlab("FITC Log") +
  theme_bw() + geom_gate(gate_r3) + geom_stats(adjust = 0.5)
...
# scale_fill_discrete(name = "", labels = c(expression("no anti-TNF"*alpha*~"staining"), expression("anti-TNF"*alpha*~"staining")))

hist_log_2A <- ggcyto(fcm2A_gate_r1, aes(x = FL.1.Log)) + geom_histogram() + scale_x_log10() +
  ylab("Count") +
  xlab("FITC Log") +
  theme_bw() + geom_gate(gate_r3) + geom_stats(adjust = 0.5)
...
hist_log_2B <- ggcyto(fcm2B_gate_r1, aes(x = FL.1.Log)) + geom_histogram() + scale_x_log10() +
  ylab("Count") +
  xlab("FITC Log") +
  theme_bw() + geom_gate(gate_r3) + geom_stats(adjust = 0.5)
...
TNFalpha_detection <- ggpubr::ggarrange(log_1A_gate, hist_log_1A, hist_log_1B, hist_log_2A, hist_log_2B, labels = "AUTO")
TNFalpha_detection
...
fcm1A_summary <- as.data.frame(fcm1A_summary[3, 12]) %>% mutate(., Mean = mean_fcm1A) %>%
  mutate(., Cell_Count = cell_count_1A) %>% rename(Median = `fcm1A_summary[3, 12]`) %>% mutate(., LPS_Stimulation = "-") %>% mutate(., Anti_TNF_Stain = "-") %>% mutate(., Cells_Positive_for_TNF = FITC_count_1A)
fcm1B_summary <- summary(fcm1B_gate_r1) 
mean_fcm1B <- fcm1B_summary[4, 12]
fcm1B_summary <- as.data.frame(fcm1B_summary[3, 12]) %>% mutate(., Mean = mean_fcm1B) %>%
  mutate(., Cell_Count = cell_count_1B) %>% rename(Median = `fcm1B_summary[3, 12]`) %>% mutate(., LPS_Stimulation = "-") %>% mutate(., Anti_TNF_Stain = "+") %>% mutate(., Cells_Positive_for_TNF = FITC_count_1B)
fcm2A_summary <- summary(fcm2A_gate_r1)
mean_fcm2A <- fcm2A_summary[4, 12]
fcm2A_summary <- as.data.frame(fcm2A_summary[3, 12]) %>% mutate(., Mean = mean_fcm2A) %>%
  mutate(., Cell_Count = cell_count_2A) %>% rename(Median = `fcm2A_summary[3, 12]`) %>% mutate(., LPS_Stimulation = "+") %>% mutate(., Anti_TNF_Stain = "-") %>% mutate(., Cells_Positive_for_TNF = FITC_count_2A)
fcm2B_summary <- summary(fcm2B_gate_r1)
mean_fcm2B <- fcm2B_summary[4, 12]
fcm2B_summary <- as.data.frame(fcm2B_summary[3, 12]) %>% mutate(., Mean = mean_fcm2B) %>%
  mutate(., Cell_Count = cell_count_2B) %>% rename(Median = `fcm2B_summary[3, 12]`) %>% mutate(., LPS_Stimulation = "+") %>% mutate(., Anti_TNF_Stain = "+") %>% mutate(., Cells_Positive_for_TNF = FITC_count_2B)

log_summary <- rbind(fcm1A_summary, fcm1B_summary, fcm2A_summary, fcm2B_summary)
log_summary <- log_summary[, c(4, 5, 3, 6, 2, 1)]

knitr::kable(log_summary) 

#still need to find a way of extracting the positive cells for TNF-alpha (also using special characters in tables)

```

```{r logicle histograms, out.width="100%"}

#setting up gating region

poly_FITC_logicle <- matrix(c(2.25, Inf,                  
                  Inf, Inf), 
                ncol = 1, 
                nrow = 2, 
                byrow = T)

colnames(poly_FITC_logicle) <- c("FL.1.Lin")
gate_r4 <- rectangleGate(poly_FITC_logicle, filterId = "gate_r4")


hist_logicle_1A <- ggcyto(fcm1A_gate_r2, aes(x = FL.1.Lin)) + geom_histogram() +
  ylab("Count") +
  xlab("FITC Lin") +
  theme_bw() + geom_gate(gate_r4) + geom_stats(adjust = 0.5)

hist_logicle_1A <- as.ggplot(hist_logicle_1A)

fcm1A_gate_r4 <- Subset(fcm1A_gate_r2, gate_r4)
fcm1A_gate_r4df <- data.frame(fcm1A_gate_r4@exprs)

hist_logicle_1B <- ggcyto(fcm1B_gate_r2, aes(x = FL.1.Lin)) + geom_histogram() +
  ylab("Count") +
  xlab("FITC Lin") +
  theme_bw() + geom_gate(gate_r4) + geom_stats(adjust = 0.5)

hist_logicle_1B <- as.ggplot(hist_logicle_1B)

fcm1B_gate_r4 <- Subset(fcm1B_gate_r2, gate_r4)
fcm1B_gate_r4df <- data.frame(fcm1B_gate_r4@exprs)

# scale_fill_discrete(name = "", labels = c(expression("no anti-TNF"*alpha*~"staining"), expression("anti-TNF"*alpha*~"staining")))

hist_logicle_2A <- ggcyto(fcm2A_gate_r2, aes(x = FL.1.Lin)) + geom_histogram() +
  ylab("Count") +
  xlab("FITC Lin") +
  theme_bw() + geom_gate(gate_r4) + geom_stats(adjust = 0.5)

hist_logicle_2A <- as.ggplot(hist_logicle_2A)

fcm2A_gate_r4 <- Subset(fcm2A_gate_r2, gate_r4)
fcm2A_gate_r4df <- data.frame(fcm2A_gate_r4@exprs)

hist_logicle_2B <- ggcyto(fcm2B_gate_r2, aes(x = FL.1.Lin)) + geom_histogram() +
  ylab("Count") +
  xlab("FITC Lin") +
  theme_bw() + geom_gate(gate_r4) + geom_stats(adjust = 0.5)

hist_logicle_2B <- as.ggplot(hist_logicle_2B)

fcm2B_gate_r4 <- Subset(fcm2B_gate_r2, gate_r4)
fcm2B_gate_r4df <- data.frame(fcm2B_gate_r4@exprs)

TNFalpha_detection_logicle <- ggpubr::ggarrange(logicle_1A_gate, hist_logicle_1A, hist_logicle_1B, hist_logicle_2A, hist_logicle_2B, labels = "AUTO")
TNFalpha_detection_logicle

ggsave("figures/Detection_of_TNFalpha_logicle.png", TNFalpha_detection_logicle)

```

```{r summary table logicle, out.width="100%"}

cell_count_1A_logicle <- nrow(fcm1A_gate_r2) 
cell_count_1B_logicle <- nrow(fcm1B_gate_r2)
cell_count_2A_logicle <- nrow(fcm2A_gate_r2)
cell_count_2B_logicle <- nrow(fcm2B_gate_r2)

FITC_count_1A_logicle <- nrow(fcm1A_gate_r4)
FITC_count_1B_logicle <- nrow(fcm1B_gate_r4)
FITC_count_2A_logicle <- nrow(fcm2A_gate_r4)
FITC_count_2B_logicle <- nrow(fcm2B_gate_r4)

fcm1A_summary_logicle <- summary(fcm1A_gate_r2) 
mean_fcm1A_logicle <- fcm1A_summary_logicle[4, 10]
fcm1A_summary_logicle <- as.data.frame(fcm1A_summary_logicle[3, 10]) %>% mutate(., Mean = mean_fcm1A_logicle) %>%
  mutate(., Cell_Count = cell_count_1A_logicle) %>% rename(Median = `fcm1A_summary_logicle[3, 10]`) %>% mutate(., LPS_Stimulation = "-") %>% mutate(., Anti_TNF_Stain = "-") %>% mutate(., Cells_Positive_for_TNF = FITC_count_1A_logicle)
fcm1B_summary_logicle <- summary(fcm1B_gate_r2) 
mean_fcm1B_logicle <- fcm1B_summary_logicle[4, 10]
fcm1B_summary_logicle <- as.data.frame(fcm1B_summary_logicle[3, 10]) %>% mutate(., Mean = mean_fcm1B_logicle) %>%
  mutate(., Cell_Count = cell_count_1B_logicle) %>% rename(Median = `fcm1B_summary_logicle[3, 10]`) %>% mutate(., LPS_Stimulation = "-") %>% mutate(., Anti_TNF_Stain = "+") %>% mutate(., Cells_Positive_for_TNF = FITC_count_1B_logicle)
fcm2A_summary_logicle <- summary(fcm2A_gate_r2)
mean_fcm2A_logicle <- fcm2A_summary_logicle[4, 10]
fcm2A_summary_logicle <- as.data.frame(fcm2A_summary_logicle[3, 10]) %>% mutate(., Mean = mean_fcm2A_logicle) %>%
  mutate(., Cell_Count = cell_count_2A_logicle) %>% rename(Median = `fcm2A_summary_logicle[3, 10]`) %>% mutate(., LPS_Stimulation = "+") %>% mutate(., Anti_TNF_Stain = "-") %>% mutate(., Cells_Positive_for_TNF = FITC_count_2A_logicle)
fcm2B_summary_logicle <- summary(fcm2B_gate_r2)
mean_fcm2B_logicle <- fcm2B_summary_logicle[4, 10]
fcm2B_summary_logicle <- as.data.frame(fcm2B_summary_logicle[3, 10]) %>% mutate(., Mean = mean_fcm2B_logicle) %>%
  mutate(., Cell_Count = cell_count_2B_logicle) %>% rename(Median = `fcm2B_summary_logicle[3, 10]`) %>% mutate(., LPS_Stimulation = "+") %>% mutate(., Anti_TNF_Stain = "+") %>% mutate(., Cells_Positive_for_TNF = FITC_count_2B_logicle)

logicle_summary <- rbind(fcm1A_summary_logicle, fcm1B_summary_logicle, fcm2A_summary_logicle, fcm2B_summary_logicle)
logicle_summary <- logicle_summary[, c(4, 5, 3, 6, 2, 1)]

knitr::kable(logicle_summary)


```


```{r bacteria uptake, out.width="100%"}
...
red_log_1A <- ggcyto(fcm1A_gate_r1, aes(x = FL.3.Log)) + geom_histogram() + scale_x_log10() +
  ylab("Count") +
  xlab("PE-Texas Red Log") +
  theme_bw() + geom_gate(gate_r5) + geom_stats(adjust = 0.5)
...
red_log_1B <- ggcyto(fcm1B_gate_r1, aes(x = FL.3.Log)) + geom_histogram() + scale_x_log10() +
  ylab("Count") +
  xlab("PE-Texas Red Log") +
  theme_bw() + geom_gate(gate_r5) + geom_stats(adjust = 0.5)
...
red_log_2A <- ggcyto(fcm2A_gate_r1, aes(x = FL.3.Log)) + geom_histogram() + scale_x_log10() +
  ylab("Count") +
  xlab("PE-Texas Red Log") +
  theme_bw() + geom_gate(gate_r5) + geom_stats(adjust = 0.5)
...
red_log_2B <- ggcyto(fcm2B_gate_r1, aes(x = FL.3.Log)) + geom_histogram() + scale_x_log10() +
  ylab("Count") +
  xlab("PE-Texas Red Log") +
  theme_bw() + geom_gate(gate_r5) + geom_stats(adjust = 0.5)
...
bacteria_detection <- ggpubr::ggarrange(log_1A_gate, red_log_1A, red_log_1B, red_log_2A, red_log_2B, labels = "AUTO")
bacteria_detection
...
fcm1A_summary_bacteria <- as.data.frame(fcm1A_summary_bacteria[3, 18]) %>% mutate(., Mean = mean_fcm1A_bacteria) %>%
  mutate(., Cell_Count = cell_count_1A_bacteria) %>% rename(Median = `fcm1A_summary_bacteria[3, 18]`) %>% mutate(., Bacteria_Stimulation = "-") %>% mutate(., Anti_TNF_Stain = "-")
fcm1B_summary_bacteria <- summary(fcm1B_gate_r5) 
mean_fcm1B_bacteria <- fcm1B_summary_bacteria[4, 18]
fcm1B_summary_bacteria <- as.data.frame(fcm1B_summary_bacteria[3, 18]) %>% mutate(., Mean = mean_fcm1B_bacteria) %>%
  mutate(., Cell_Count = cell_count_1B_bacteria) %>% rename(Median = `fcm1B_summary_bacteria[3, 18]`) %>% mutate(., Bacteria_Stimulation = "-") %>% mutate(., Anti_TNF_Stain = "+") 
fcm2A_summary_bacteria <- summary(fcm2A_gate_r5)
mean_fcm2A_bacteria <- fcm2A_summary_bacteria[4, 18]
fcm2A_summary_bacteria <- as.data.frame(fcm2A_summary_bacteria[3, 18]) %>% mutate(., Mean = mean_fcm2A_bacteria) %>%
  mutate(., Cell_Count = cell_count_2A_bacteria) %>% rename(Median = `fcm2A_summary_bacteria[3, 18]`) %>% mutate(., Bacteria_Stimulation = "+") %>% mutate(., Anti_TNF_Stain = "-") 
fcm2B_summary_bacteria <- summary(fcm2B_gate_r5)
mean_fcm2B_bacteria <- fcm2B_summary_bacteria[4, 18]
fcm2B_summary_bacteria <- as.data.frame(fcm2B_summary_bacteria[3, 18]) %>% mutate(., Mean = mean_fcm2B_bacteria) %>%
  mutate(., Cell_Count = cell_count_2B_bacteria) %>% rename(Median = `fcm2B_summary_bacteria[3, 18]`) %>% mutate(., Bacteria_Stimulation = "+") %>% mutate(., Anti_TNF_Stain = "+") 

bacteria_summary <- rbind(fcm1A_summary_bacteria, fcm1B_summary_bacteria, fcm2A_summary_bacteria, fcm2B_summary_bacteria)
bacteria_summary <- bacteria_summary[, c(4, 5, 3, 2, 1)]

knitr::kable(bacteria_summary)

```


```{r logicle bacteria uptake, out.width="100%"}

#setting up gating region

poly_FITC <- matrix(c(2.25, Inf,                  
                  Inf, Inf), 
                ncol = 1, 
                nrow = 2, 
                byrow = T)

colnames(poly_FITC) <- c("FL.3.Lin")
gate_r6 <- rectangleGate(poly_FITC, filterId = "gate_r6")


red_logicle_1A <- ggcyto(fcm1A_gate_r2, aes(x = FL.3.Lin)) + geom_histogram() + 
  ylab("Count") +
  xlab("PE-Texas Red Lin") +
  theme_bw() + geom_gate(gate_r6) + geom_stats(adjust = 0.5)

red_logicle_1A <- as.ggplot(red_logicle_1A)

fcm1A_gate_r6 <- Subset(fcm1A_gate_r2, gate_r6)
fcm1A_gate_r6df <- data.frame(fcm1A_gate_r6@exprs)

red_logicle_1B <- ggcyto(fcm1B_gate_r2, aes(x = FL.3.Lin)) + geom_histogram() + 
  ylab("Count") +
  xlab("PE-Texas Red Lin") +
  theme_bw() + geom_gate(gate_r6) + geom_stats(adjust = 0.5)

red_logicle_1B <- as.ggplot(red_logicle_1B)

fcm1B_gate_r6 <- Subset(fcm1B_gate_r2, gate_r6)
fcm1B_gate_r6df <- data.frame(fcm1B_gate_r6@exprs)

# scale_fill_discrete(name = "", labels = c(expression("no anti-TNF"*alpha*~"staining"), expression("anti-TNF"*alpha*~"staining")))

red_logicle_2A <- ggcyto(fcm2A_gate_r2, aes(x = FL.3.Lin)) + geom_histogram() + 
  ylab("Count") +
  xlab("PE-Texas Red Lin") +
  theme_bw() + geom_gate(gate_r6) + geom_stats(adjust = 0.5)

red_logicle_2A <- as.ggplot(red_logicle_2A)

fcm2A_gate_r6 <- Subset(fcm2A_gate_r2, gate_r6)
fcm2A_gate_r6df <- data.frame(fcm2A_gate_r6@exprs)

red_logicle_2B <- ggcyto(fcm2B_gate_r2, aes(x = FL.3.Lin)) + geom_histogram() + 
  ylab("Count") +
  xlab("PE-Texas Red Lin") +
  theme_bw() + geom_gate(gate_r6) + geom_stats(adjust = 0.5)

red_logicle_2B <- as.ggplot(red_logicle_2B)

fcm2B_gate_r6 <- Subset(fcm2B_gate_r2, gate_r6)
fcm2B_gate_r6df <- data.frame(fcm2B_gate_r6@exprs)

bacteria_detection_logicle <- ggpubr::ggarrange(logicle_1A_gate, red_logicle_1A, red_logicle_1B, red_logicle_2A, red_logicle_2B, labels = "AUTO")
bacteria_detection_logicle

ggsave("figures/Detection_of_Bacteria_logicle.png", bacteria_detection_logicle)
