...
<!-- # 1A: not stained -->
<!-- # 1B: stained for intracellular TNF production: detected with an anti-rabbit 488 (Green)# # -->

<!-- **Sample 2** treated with Brefeldin A and red-fluorescent (rhodamine) bacteria (stimulated LPS treated) -->
<!-- # 2A: not stained  -->
<!-- # 2B: stained for intracellular TNF production: detected with an anti-rabbit 488 (Green) -->
...
```{r compensating the data}

spillover(fcm1A) #exists in the $spillover keyword
...
spillover(fcm1B) #exists in the $spillover keyword
...
spillover(fcm2A) #exists in the $spillover keyword
...
spillover(fcm2B) #exists in the $spillover keyword
...
trans1A <- estimateLogicle(fcm1A_comp, colnames(fcm1A_comp[, 4:36]))
fcm1A_comp_trans <- transform(fcm1A_comp, trans1A)

trans1B <- estimateLogicle(fcm1B_comp, colnames(fcm1B_comp[, 4:36]))
fcm1B_comp_trans <- transform(fcm1B_comp, trans1B)

trans2A <- estimateLogicle(fcm2A_comp, colnames(fcm2A_comp[, 4:36]))
fcm2A_comp_trans <- transform(fcm2A_comp, trans2A)

trans2B <- estimateLogicle(fcm2B_comp, colnames(fcm2B_comp[, 4:36]))
...
autoplot(fcm1A_comp) # doesn't visualise very well without transformation
autoplot(fcm1A_comp_trans)
autoplot(fcm1B_comp_trans)
autoplot(fcm2A_comp_trans)
autoplot(fcm2B_comp_trans)

ggcyto(fcm1A_comp_trans, aes(x = "FS.Lin", y = "SS.Lin")) + geom_hex(bins = 256) + theme_bw()

ggcyto(fcm1B_comp_trans, aes(x = "FS.Lin", y = "SS.Lin")) + geom_hex(bins = 256) + theme_bw()

ggcyto(fcm2A_comp_trans, aes(x = "FS.Lin", y = "SS.Lin")) + geom_hex(bins = 256) + theme_bw()

ggcyto(fcm2B_comp_trans, aes(x = "FS.Lin", y = "SS.Lin")) + geom_hex(bins = 256) + theme_bw()
