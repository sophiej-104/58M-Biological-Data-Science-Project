...
ggpubr::ggarrange(log_1A_gate, log_1B_gate, log_2A_gate, log_2B_gate, labels = "AUTO")

```

```{r setting the initial gating regions for logicle data, include=FALSE, out.width="100%"}
...
log_summary <- log_summary[, c(4, 5, 3, 6, 7, 2, 1)]

knitr::kable(log_summary, caption = "Summary statistics obtained from flow cytometry analysis comparing the production of fluorecently labelled TNF-$\\alpha$ by macrophages stimulated with (+) or without (-) LPS or red fluorescent (rhodamine) bacteria", col.names = gsub("[_]", " ", names(log_summary)))

#still need to find a way of extracting the positive cells for TNF-alpha (also using special characters in tables)

```

```{r logicle histograms, include=FALSE, out.width="100%"}
...
```{r summary table logicle, include=FALSE, out.width="100%"}
...
```{r bacteria-uptake, out.width="100%",fig.cap='(ref:bacteria-uptake)'}
...
fcm1A_summary_bacteria <- as.data.frame(fcm1A_summary_bacteria[3, 18]) %>% mutate(., Mean_PE_Texas_Red_Fluorescence_Intensity = mean_fcm1A_bacteria) %>%
  mutate(., Cell_Count = cell_count_1A_bacteria) %>% rename(Median_PE_Texas_Red_Fluorescence_Intensity = `fcm1A_summary_bacteria[3, 18]`) %>% mutate(., Bacteria_Stimulation = "-") %>% mutate(., Anti_TNF_Stain = "-")
...
fcm1B_summary_bacteria <- as.data.frame(fcm1B_summary_bacteria[3, 18]) %>% mutate(., Mean_PE_Texas_Red_Fluorescence_Intensity = mean_fcm1B_bacteria) %>%
  mutate(., Cell_Count = cell_count_1B_bacteria) %>% rename(Median_PE_Texas_Red_Fluorescence_Intensity = `fcm1B_summary_bacteria[3, 18]`) %>% mutate(., Bacteria_Stimulation = "-") %>% mutate(., Anti_TNF_Stain = "+") 
...
fcm2A_summary_bacteria <- as.data.frame(fcm2A_summary_bacteria[3, 18]) %>% mutate(., Mean_PE_Texas_Red_Fluorescence_Intensity = mean_fcm2A_bacteria) %>%
  mutate(., Cell_Count = cell_count_2A_bacteria) %>% rename(Median_PE_Texas_Red_Fluorescence_Intensity = `fcm2A_summary_bacteria[3, 18]`) %>% mutate(., Bacteria_Stimulation = "+") %>% mutate(., Anti_TNF_Stain = "-") 
...
fcm2B_summary_bacteria <- as.data.frame(fcm2B_summary_bacteria[3, 18]) %>% mutate(., Mean_PE_Texas_Red_Fluorescence_Intensity = mean_fcm2B_bacteria) %>%
  mutate(., Cell_Count = cell_count_2B_bacteria) %>% rename(Median_PE_Texas_Red_Fluorescence_Intensity = `fcm2B_summary_bacteria[3, 18]`) %>% mutate(., Bacteria_Stimulation = "+") %>% mutate(., Anti_TNF_Stain = "+") 
...
knitr::kable(bacteria_summary, caption = "Summary statistics obtained from flow cytometry analysis comparing the uptake of bacteria and TNF-$\\alpha$ production in macrophages stimulated with (+) or without (-) LPS or red fluorescent (rhodamine) bacteria", col.names = gsub("[_]", " ", names(bacteria_summary)))

```


```{r logicle bacteria uptake, include=FALSE, out.width="100%"}
...
comp_log_1A <- ggcyto(fcm1A_gate_r1, aes(x = FL.1.Log, y = FL.3.Log)) + geom_hex(bins = 128) + scale_x_log10() + scale_y_log10() +
  ...
  xlab("FITC Log") +
  theme_bw() + geom_gate(gate_r7) + geom_stats()
...
comp_figure <- ggpubr::ggarrange(log_1A_gate, comp_log_1A, comp_log_1B, comp_log_2A, comp_log_2B, labels = "AUTO", ncol = 2, nrow = 3, vjust = .75)
comp_figure
